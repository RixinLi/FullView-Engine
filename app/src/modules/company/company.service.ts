  ...
  async createOneCompany(@Body() body:CreateCompanyDto): Promise<InsertResult>{
      try {
        ...
        if (error.code === 'ER_DUP_ENTRY') {
          throw new HttpException(Result.error('公司唯一ID已存在',HttpStatus.CONFLICT.toString()), HttpStatus.CONFLICT);
        }
        throw new HttpException(
          Result.error('数据库错误'),
          HttpStatus.INTERNAL_SERVER_ERROR
  ...
  async updateOneCompany(@Body() body:UpdateCompanyDto): Promise<UpdateResult>{
      try {
        ...
        if (!body.company_code) {
          throw new HttpException(Result.error('缺少公司的ID',HttpStatus.BAD_REQUEST.toString()), HttpStatus.BAD_REQUEST);
        }
        ...
        if (result.affected === 0) {
          throw new HttpException(Result.error('未找到公司或更新失败',HttpStatus.NOT_FOUND.toString()), HttpStatus.NOT_FOUND);
        }
        return result;
      } catch (error) {
  ...
  */
  async deleteOneCompany(@Param('company_code') company_code:string): Promise<DeleteResult>{
      try {
        // 判断是否为空
        if (!company_code) {
          throw new HttpException(Result.error('公司代码不能为空',HttpStatus.BAD_REQUEST.toString()), HttpStatus.BAD_REQUEST);
        }
        // 数据库进行更新操作
        const result =  await this.companyRepository.delete({company_code:company_code});

        // 操作result
        if (result.affected === 0) {
          throw new HttpException(Result.error('未找到公司或删除失败',HttpStatus.NOT_FOUND.toString()), HttpStatus.NOT_FOUND);
        }
        return result;
      } catch (error) {
        ...
        if (error.code === 'ER_ROW_IS_REFERENCED_2') {
          throw new HttpException(Result.error('公司仍被引用，无法删除',HttpStatus.CONFLICT.toString()), HttpStatus.CONFLICT);
        }
        throw new HttpException(
          Result.error('数据库错误'),
          HttpStatus.INTERNAL_SERVER_ERROR
